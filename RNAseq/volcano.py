...
#import datasets
RNAseq_PAX3_BMP_filt_fpkm = pd.read_csv('./datasets/fpkm_filtered/RNAseq_PAX3_BMP_fpkm_filt.csv',sep=';')
RNAseq_PAX3_NO_filt_fpkm = pd.read_csv('./datasets/fpkm_filtered/RNAseq_PAX3_NO_fpkm_filt.csv',sep=';')
...
def volcano_plot(data, condition_1, condition_2, fc_threshold, 
    ...
    plt.text(3, 22, str(sum(mask_up)), fontsize=24, 
    ...
    plt.text(-3, 22, str(sum(mask_down)), fontsize=24, 
...
###############################################################################
'''
Section 3: Volcano plots (important to give data already filtered for fpkm values)
'''
#BMP hom vs ctl
